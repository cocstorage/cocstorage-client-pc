...
type ConditionalCommentFormProps =
  | {
      type: 'storageBoard';
      path: string;
      id: number;
...
function CommentForm({ path, id }: ConditionalCommentFormProps) {
  const {
    theme: {
      palette: { box }
    }
  ...
  const { data: { id: storageId } = {} } = useStorage(String(path));

  const storageBoard = useStorageBoardData(Number(id));

  const { data: { comments = [], pagination: { perPage = 0 } = {} } = {} } =
    useStorageBoardComments(storageId as number, Number(id), params, {
  ...
  const { mutate, isLoading } = useMutation(
    (data: PostStorageBoardCommentData) =>
      postNonMemberStorageBoardComment(storageId as number, Number(id), data),
    {
      onSuccess: () => {
        setContent('');
        ...
        if (params.page === commentLatestPage && comments.length + 1 <= perPage) {
          queryClient
            .invalidateQueries(
              queryKeys.storageBoardComments.storageBoardCommentsByIdWithParams(Number(id), params)
            )
            .then();
        } else {
