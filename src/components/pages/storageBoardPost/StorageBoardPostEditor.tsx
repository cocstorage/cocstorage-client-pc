...
function StorageBoardPostEditor() {
  ...
  const [subject, setSubject] = useRecoilState(storageBoardPostSubjectState);
  const [draftId, setDraftId] = useRecoilState(storageBoardPostDraftIdState);
  ...
  const { mutate } = useMutation((storageId: number) => postNonMemberStorageBoardDraft(storageId), {
    onSuccess: ({ id: newDraftId }) => setDraftId(newDraftId)
  });
